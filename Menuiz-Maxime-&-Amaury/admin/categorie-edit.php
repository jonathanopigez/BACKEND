...
if (empty($errors)) {
    if (isset($_GET['id'])) { // modification
        $query = 'UPDATE T_D_PRODUCTTYPE_PTY SET PTY_DESCRIPTION = :nom WHERE PTY_ID = :id';
        ...
        // insertion en bdd
        $stmt = $pdo->prepare('INSERT INTO T_D_PRODUCTTYPE_PTY(PTY_DESCRIPTION) VALUES (:nom)');
        $stmt = $pdo->prepare($query);
    ...
    $query = 'SELECT * FROM T_D_PRODUCTTYPE_PTY WHERE PTY_ID=' . (int)$_GET['id'];
    $stmt = $pdo->query($query);
    $categorie = $stmt->fetch();
    $nom = $categorie['PTY_DESCRIPTION'];
}
